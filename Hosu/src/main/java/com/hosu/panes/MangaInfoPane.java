...
public class MangaInfoPane extends Pane{
	...
	public void setMangaData(QueriedManga manga) {
	...
	public javafx.scene.layout.Pane get() {
		...
		try {
			...
			InputStream stream = Encoder.openInputStream(this.manga.getManga().getImage());
	...
	public StackPane getTextPane(){
		...
		Map<String, String> mapping = this.manga.getAttributes();
		...
		load.setOnAction(e -> {
			...
			new Thread(() -> {
				...
				try {
					...
					System.out.println("looking for " + this.manga.getManga().getAlt());
					...
					List<MangaSearchSub> results = (List<MangaSearchSub>) jikan
							...
							.manga()
							.search()
					        .query(this.manga.getManga().getAlt())
					        .execute()
					        .collectList()
