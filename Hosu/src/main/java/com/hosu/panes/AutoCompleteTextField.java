...
public class AutoCompleteTextField extends TextField {
    ...
    private void populatePopup(List<String> searchResult, String text) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            entryFlow.getStylesheets().add(HosuClient.getInstance().getCssManager().getCss(Styling.HOSU));
            entryFlow.setId("flow1");
            ...
            menuItems.add(item);
            entryFlow.setCursor(Cursor.HAND);
        }
        
        entriesPopup.getItems().clear();
        entriesPopup.getItems().addAll(menuItems);
        entriesPopup.setId("flow2");
    }
